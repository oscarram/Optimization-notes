...
\begin{remark}[Notation]
	The result of applying the operator $\mathsf{D}F(x)$ to some vector $y$, (derivative at $x$ in direction $y$)  will be denote by apostrophe to the function $F'(x; y)=\mathsf{D}F(x)y$. If we are in a Hilbert Space $(H, \langle \cdot \rangle)$ by Riesz representation formula and the fact $\mathsf{D}F(x) \in \mathcal{B}(H,H)$, we have
	\[
